...
class SEOAnalyzer:
    ...
    def _find_pages(self):
        ...
        page_patterns = {
            "next.js": [
                ...
                "pages/**/*.tsx", "pages/**/*.jsx", "pages/**/*.js", "pages/**/*.ts"
            ],
            "astro": [
    ...
    def _determine_page_type(self, path: Path) -> str:
        ...
        if "faq" in path_str:
            return "faq"
        if path.name in [
            "index.html", "page.tsx", "page.jsx", "page.js", "page.ts",
            "index.astro", "index.vue"
        ]:
    ...
    def _collect_shared_metadata(self):
        """Collect layout and shared SEO files that may define metadata for many pages."""
        candidate_patterns = {
            "next.js": [
                "app/**/layout.*", "app/**/head.*", "pages/_app.*", "pages/_document.*",
                "components/**/*SEO*.*", "components/**/*Seo*.*",
                "src/components/**/*SEO*.*", "src/components/**/*Seo*.*"
            ],
            "astro": [
                "src/layouts/**/*.*", "src/components/**/*SEO*.*", "src/components/**/*Seo*.*"
            ],
            "gatsby": [
                "src/components/**/*SEO*.*", "src/components/**/*Seo*.*",
                "src/templates/**/*.*"
            ...
            "nuxt": [
                "app.vue", "layouts/**/*.vue", "app/layouts/**/*.vue",
                "components/**/*SEO*.*", "components/**/*Seo*.*"
            ],
            "react": [
                "src/App.*", "src/components/**/*SEO*.*", "src/components/**/*Seo*.*"
            ],
            "vue": [
                "src/App.vue", "src/components/**/*SEO*.*", "src/components/**/*Seo*.*"
            ],
            "static": []
        }

        patterns = candidate_patterns.get(self.framework, [])
        files = []
        chunks = []
        seen = set()
        ...
        for pattern in patterns:
            for path in self.project_path.glob(pattern):
                if not path.is_file() or path in seen or self._is_excluded_path(path):
                    continue
                seen.add(path)
                try:
                    ...
                    continue
                if not self._contains_metadata_signal(content):
                    continue
                files.append(str(path.relative_to(self.project_path)))
                chunks.append(content)

        self.shared_metadata_content = "\n".join(chunks)
        self.findings["shared_metadata_files"] = files

        if files:
            print(f"Shared metadata files found: {len(files)}")
    ...
    def _is_excluded_path(self, path: Path) -> bool:
        """Skip generated, dependency, and build output paths."""
        path_str = str(path.relative_to(self.project_path))
        return any(
            part in path_str
            for part in ["node_modules/", ".next/", "dist/", "build/", ".astro/", ".nuxt/"]
        )

    def _contains_metadata_signal(self, content: str) -> bool:
        """Return true if a file appears to define SEO metadata."""
        signals = [
            "metadata", "generateMetadata", "<Head", "next/head", "Helmet",
            "useHead", "useSeoMeta", "useServerSeoMeta", "<title>",
            "description", "openGraph", "og:", "twitter:", "canonical",
            "application/ld+json", "@context"
        ]
        content_lower = content.lower()
        return any(signal.lower() in content_lower for signal in signals)
    ...
    def _check_sitemap(self):
        ...
        if self.framework == "nuxt":
            for config_file in ["nuxt.config.ts", "nuxt.config.js", "nuxt.config.mjs"]:
                ...
                if config_path.exists():
                    ...
                    if "sitemap" in content.lower():
                        ...
                        print(f"Sitemap: Found sitemap configuration in {config_file}")
                        return
    ...
    def _analyze_meta_tags(self):
        ...
        for page in self.pages[:20]:  # Analyze first 20 pages
            content = Path(page["full_path"]).read_text(errors="ignore")
            combined_content = content + "\n" + self.shared_metadata_content

            has_title = self._check_meta_tag(combined_content, "title")
            has_desc = self._check_meta_tag(combined_content, "description")
            has_og = self._check_meta_tag(combined_content, "og:")
            has_twitter = self._check_meta_tag(combined_content, "twitter:")
            has_canonical = self._check_meta_tag(combined_content, "canonical")
        ...
        meta_findings["pages_analyzed"] = pages_analyzed
    ...
    def _check_meta_tag(self, content: str, tag_type: str) -> bool:
        ...
        patterns = {
            "title": [
                r"<title>", r"title:", r"title=", r'"title":', r"useSeoMeta", r"useServerSeoMeta"
            ],
            "description": [
                r'name="description"', r'name=\'description\'',
                r"description:", r'"description":', r"useSeoMeta", r"useServerSeoMeta"
            ],
            "og:": [
                r'property="og:', r"property='og:", r"openGraph",
                r"ogTitle", r"ogDescription", r"ogImage", r"ogUrl"
            ],
            "twitter:": [
                r'name="twitter:', r"name='twitter:", r"twitter:",
                r"twitterCard", r"twitterTitle", r"twitterDescription", r"twitterImage"
            ],
            "canonical": [
    ...
    def _get_recommendations(self) -> List[Dict[str, str]]:
        ...
        meta = self.findings.get("meta_tags", {})
        pages_analyzed = meta.get("pages_analyzed", len(self.pages))
        if meta.get("pages_with_description", 0) < pages_analyzed:
            recommendations.append({
        ...
        if meta.get("pages_with_og", 0) < pages_analyzed:
            recommendations.append({
