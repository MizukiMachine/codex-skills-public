...
class SitemapGenerator:
    ...
    def _discover_nextjs_routes(self):
        ...
        if app_dir.exists():
            for ext in PAGE_EXTENSIONS:
                for page_file in app_dir.glob(f"**/page.{ext}"):
                    route = self._nextjs_app_path_to_route(page_file, app_dir)
    ...
    def _append_route(self, route: Optional[Dict]):
        """Append a route once, preserving the first discovered file metadata."""
        if not route or route["url"] in self.route_urls:
            return
        self.routes.append(route)
        self.route_urls.add(route["url"])

    def _nextjs_app_path_to_route(self, page_file: Path, app_dir: Path) -> Optional[Dict]:
        ...
        for segment in segments:
            if segment in (".", ""):
                continue
            if self._should_skip_nextjs_app_segment(segment):
                return None
        ...
        if not route_segments:
            ...
            url_path = "/" + "/".join(route_segments)

        return self._create_route_entry(url_path, page_file)

    def _should_skip_nextjs_app_segment(self, segment: str) -> bool:
        """Return true for segments that should not become public sitemap URLs."""
        if segment == "api" or segment.startswith("_") or segment.startswith("@"):
            return True
    ...
    def _nextjs_pages_to_route(self, page_file: Path, pages_dir: Path) -> Optional[Dict]:
        ...
        if (
            parts[0] == "api"
            or page_file.stem in {"_app", "_document", "_error", "404", "500"}
        ...
        url_path = re.sub(r"\.(js|jsx|ts|tsx)$", "", url_path)
    ...
    def _discover_nuxt_routes(self):
        """Discover static Nuxt routes from pages directories."""
        for pages_dir in [self.project_path / "pages", self.project_path / "app" / "pages"]:
            if not pages_dir.exists():
                continue

            for page_file in pages_dir.glob("**/*.vue"):
                route = self._nuxt_path_to_route(page_file, pages_dir)
                self._append_route(route)

    def _nuxt_path_to_route(self, page_file: Path, pages_dir: Path) -> Optional[Dict]:
        """Convert a Nuxt pages path to a static route."""
        relative = page_file.relative_to(pages_dir)
        path_str = str(relative)

        # Dynamic and catch-all routes require data and should not be guessed.
        if any(x in path_str for x in ["[", "]", "_", "+"]):
            return None

        url_path = "/" + path_str.replace("\\", "/")
        url_path = re.sub(r"\.vue$", "", url_path)
        url_path = re.sub(r"/index$", "/", url_path)
