# Phaser Mobile Runtime Patterns for Capacitor Android

Use this when editing Phaser game code that must behave correctly inside an Android WebView.
...
rg --files | rg '(^|/)(capacitor.config|vite.config|src|public|assets|static|maps|tilemaps|textures|audio)'
```
...
- Scale mode, virtual size, CSS container rules, pixel-art flags, orientation assumptions, and camera bounds.
- Input model, active pointer count, virtual controls, keyboard fallback, and pause/back-button behavior.
...
- Check texture bleeding in atlases and tilemaps on device screenshots.
...
this.load.audio('jump', ['/assets/audio/jump.webm', '/assets/audio/jump.mp3']);
```
...
- Case mismatches that work on a permissive local filesystem but fail in Android packaging or CI.

Add loader diagnostics during Android debugging:
...
PauseScene     # pause menu and Android back-button target
```
...
## Capacitor Pause, Resume, and Back Button

Use `@capacitor/app` when native lifecycle or hardware back behavior matters:

```ts
import { App } from '@capacitor/app';

export function installAndroidLifecycle(game: Phaser.Game) {
...
Only register `backButton` when the game has a product rule for it. Listening for this event disables Capacitor's default back-button handling, so always route to in-game behavior, browser history, or an explicit app exit:
...
- Keep touch target sizes usable on small screens.
...
- Treat notches and Android navigation bars as layout constraints when UI sits near edges.
...
Use short, compressed audio for mobile. Include a fallback format when the project already uses multiple formats. Pause or mute long-running sounds during app backgrounding.

## WebGL Context and Render Stability

Phaser handles common renderer setup, but Android can still expose memory and context issues:
...
- Surface WebGL errors with WebView inspect rather than guessing from a blank screen.
...
For Android-facing Phaser changes, verify:
- Production `npm run build` succeeds.
- `npx cap sync android` runs after build.
- Device/emulator shows a nonblank canvas.
- Phaser loader errors are absent in WebView console.
- Touch controls work with multi-touch where required.
- Audio starts after first gesture and pauses/resumes correctly.
- Orientation and scale remain correct after rotation or resume.
- The hardware back button follows the product rule.
