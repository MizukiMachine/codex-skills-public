...
- [Philosophy: Measure Before You Code](#philosophy-measure-before-you-code)
- [Spritesheet Loading](#spritesheet-loading)
- [Asset Inspection Protocol](#asset-inspection-protocol)
- [Nine-Slice UI Panels](#nine-slice-ui-panels)
- [Common Pitfalls](#common-pitfalls)
- [Debugging Spritesheets](#debugging-spritesheets)
- [Asset Documentation Template](#asset-documentation-template)
...
**Check Square Frames as One Candidate**

Many character spritesheets use **square frames** (frameWidth = frameHeight), but non-square character frames are also common. Treat square frames as a hypothesis to verify, not a rule.

```
Image: 448×392 pixels

Bad shortcut (assuming 8x8 grid without measuring):
  - 448/8 = 56 (width) ✓
  - 392/8 = 49 (height) ← Produces bleeding artifacts!

Better approach (test measured candidates):
...
2. Test whether the same value also explains frameHeight cleanly
3. Check if height divides evenly: `imageHeight / frameWidth = integer?`
4. Use non-square frame dimensions when measurement or animation layout proves them
...
- **frameWidth ≠ frameHeight without measurement** - non-square character frames are valid, but should be measured rather than inferred from row count
...
### Built-In NineSlice First

Prefer Phaser's built-in `NineSlice` Game Object when the project version and renderer support it. Phaser 3.60+ includes a WebGL `NineSlice`; Phaser 3.70+ can read slice data from TexturePacker atlas JSON when it is exported with the frame.

```javascript
this.load.image('dialog-panel', 'assets/ui/dialog-panel.png');
...
Use manual assembly only when the project is on an older Phaser version, needs a Canvas fallback, or the source art has transparent/internal padding that the built-in object cannot represent cleanly.
