...
Strategies for maintaining a stable frame rate in Phaser 3 and Phaser 4 games.
...
1. Reproduce the slow scene with a consistent camera, player state, and enemy/object count.
2. Measure `this.game.loop.actualFps`, Chrome Performance, memory, and object counts before changing code.
3. Fix object churn and collision-pair explosions before micro-optimizing rendering.
4. Re-test at desktop and mobile-sized viewports.

For Phaser 4 projects, consider v4-specific GPU-oriented features such as SpriteGPULayer or TilemapGPULayer only after proving ordinary batching, atlases, pooling, and culling are not enough.
