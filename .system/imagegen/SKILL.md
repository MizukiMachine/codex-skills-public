---
name: "imagegen"
description: "写真、イラスト、テクスチャ、スプライト、モックアップ、透明背景の切り抜きなど、AI生成のラスタ画像が役立つタスクで画像を生成または編集する。Codexが新規画像を作成する、既存画像を変換する、参照から派生バリエーションを作る、出力がリポジトリ内コードやベクターではなくビットマップ資産である場合に使う。既存SVG、ベクター、コード由来アセット、既存アイコン/ロゴ体系の拡張、HTML/CSS/canvasで直接作る方が適切な場合は使わない。"
---
