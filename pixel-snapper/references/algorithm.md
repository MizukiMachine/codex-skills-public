...
The snapper recovers the true pixel grid from an upscaled/AI-faked pixel-art image in five stages. This file documents the pipeline at the level needed to debug bad outputs or extend the script.
...
- Initialization: `k` random pixels picked via numpy `default_rng(seed=42)`.
